...
func TestImport(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []*mavseal.PublicKey{imported}, list)
}
...
func TestPolicy(t *testing.T) {
	type testCase struct {
		...
		policy   mavseal.PublicKeyPolicy
		expected string
	}
	...
	for _, c := range cases {
		t.Run(c.title, func(t *testing.T) {
			conf := mavseal.Config{
				...
				Policy: hashmap.NewPublicKeyHashMap([]hashmap.PublicKeyKV[*mavseal.PublicKeyPolicy]{{Key: pk.Hash(), Val: &c.policy}}),
			}
			...
			_, err = s.Sign(context.Background(), &mavseal.SignRequest{PublicKeyHash: pk.Hash(), Message: msg})
			if c.expected != "" {
