...
MavSeal is a remote signing daemon that allows Mavryk bakers to sign endorsement and baking operations with various key-management systems.

MavSeal currently supports [AWS KMS][aws], [Azure Key Vault][azure], [GCP Key Management][gcp], [YubiHSM][yubi], [Hashicorp Vault][hashicorp], and for development/prototyping purposes, MavSeal can sign with a [local private key](localsecret.md).
...
```mermaid
sequenceDiagram
    Remote Signer Client-->>+MavSeal: mavryk operation 
    MavSeal-->>+MavSeal: checks policy
    MavSeal-->>+Vault: mavryk operation
    Vault->>+MavSeal: signature
    MavSeal->>+Remote Signer Client: signature
```

## Configuration

MavSeal configuration is specified in a YAML file. Use the `mavseal.yaml` file as a template to get started.
...
The configuration file is shared between `mavseal` and `mavseal-cli`.

### Configuration Example - File-based Vault

> :warning: **We don't recommend using the `file` Vault driver in Production.** 

The file-based vault relies on a file containing your wallet's private keys. The `file` vault driver is the least secure of them all compared to the YubiHSM and Cloud KMS drivers; However, it's the easiest Vault to set up when trying MavSeal for the first time as it doesn't require you to have a YubiHSM nor a Cloud provider account. 

`/etc/mavseal/mavseal.yaml`
```yaml
server:
...
mavryk:
  ...
  mv3CXEtMtTwnnroNp3fxkWsBV1M5xP9uMCmm:
    # Setting `log_payloads` to `true` will cause MavSeal to log operation
...
Watermarks are a critical safety feature in MavSeal that prevent double-signing operations at the same block level or round. They track the highest level/round that has been signed for each key and operation type, rejecting any requests to sign at or below that level.
...
For distributed validator setups where multiple MavSeal instances share signing responsibilities, the AWS DynamoDB backend is recommended.
...
MavSeal service is used for signing operations and implements Mavryk specific HTTP external signer API
...
Flags:
  -c, --config string   Config file path (default "mavseal.yaml")
  -h, --help            help for mavseal
      --log string      Log level: [error, warn, info, debug, trace] (default "info")
```

### Prometheus metrics and health service

MavSeal exposes Prometheus metrics and health status on the address specified in `utility_address` configuration parameter. The default value is `:9583`.
...
To test the signing operation, you can send a post to MavSeal. In this example, we are sending a dummy operation of type `02`, which is an `endorsement` operation type.
...
## MavSeal command line tool

MavSeal service is used for importing of private keys and obtaining information about available key pairs and their policies.

```
A Mavryk Remote Signer for signing block-chain operations with private keys

Usage:
  mavseal-cli import [flags]
...
Global Flags:
  ...
  -c, --config string     Config file path (default "/etc/mavseal.yaml")
...
```sh
mavseal-cli -c CONFIG import --vault VAULT PRIVATE_KEY
```

Example:

```sh
mavseal-cli -c mavseal.yaml import --vault yubi edsk3rsARzj7f8PEHXXUbLigMDCww75nPnzbFmSz19TLwzrYzF8uCB
```

### List keys

```sh
mavseal-cli -c CONFIG list
```
...
The hook is called after the standard request type and operation checks. If the hook returns an error, MavSeal denies signing the operation.
